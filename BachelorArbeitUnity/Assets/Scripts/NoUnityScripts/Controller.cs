...
namespace BachelorArbeitCSharp
{
	public class Controller
	{
		...
		public Vertex[] addVerticesBetween (Vertex v1, double[] direction, Edge edge, Mesh newMesh)
		{
			...
			for (int i = 0; i < edge.getSepNumber () - 1; i++) {
				x = v1.getX () + (direction [0] / sepNumber * (i + 1));
				y = v1.getY () + (direction [1] / sepNumber * (i + 1));
				z = v1.getZ () + (direction [2] / sepNumber * (i + 1));

				newVertices [i] = newMesh.addVertex (x, y, z);
			}
        ...
        public ObjMesh createGrid(double min, double max, double step)
        {
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                {
                    double[] v = { min + (j * step), min + (i * step), 0 };
                    vertices.Add(v);
                }
            }
            for (int i = 0; i < steps; i++)
        ...
        public double functionToFit(double x, double y)
        {
            return (25.0 - (x * y)) / 5;
        }

        public double functionToFit(Vertex v)
        {
            return functionToFit(v.getX(), v.getY());
        }
    }
}
