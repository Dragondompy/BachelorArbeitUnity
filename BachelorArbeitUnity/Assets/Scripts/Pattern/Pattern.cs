...
namespace BachelorArbeitUnity
{
    public class Pattern
    {
        ...
        public void fillPatch(MeshStruct oldMesh, MeshStruct newMesh, Face face)
        {
        ...
        public void addOuterFlowVertices(MeshStruct oldMesh, MeshStruct newMesh, Face face)
        {
        ...
        public int getVertexAt(HalfEdge prevHe, HalfEdge curHe, HalfEdge nextHe, int flow, int index)
        {
            if (flow == 0)
            {
                if (index == 0)
                {
                    return curHe.getNewV1().getHandleNumber();
                }
                else if (index == curHe.getSepNumber())
                {
                    return curHe.getNewV2().getHandleNumber();
                }
                else
                {
