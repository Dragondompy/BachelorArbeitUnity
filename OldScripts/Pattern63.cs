...
namespace BachelorArbeitCSharp
{
	public class Pattern63 : EmptyPattern
	{
		...
		public override void createFaces (Mesh newMesh, Face f,List<List<int>> innerListVertices)
		{
			...
			double[] pos5 = newMesh.getVertexAt (v6).getPosition ();
			double[] dir1 = direction (v2, middlePoint (v1, v7, newMesh), newMesh);
			double[] dir2 = direction (v3, middlePoint (v1, v7, newMesh), newMesh);
			double[] dir3 = direction (v4, middlePoint (v1, v7, newMesh), newMesh);
			double[] dir4 = direction (v5, middlePoint (v1, v7, newMesh), newMesh);
			double[] dir5 = direction (v6, middlePoint (v1, v7, newMesh), newMesh);
			for (int i = 1; i <= x+1; i++) {
